...
def get_tile_names_at_location(x, y, game_map):
    from tile_types import tile_colors
    if not game_map.in_bounds(x, y) or not game_map.visible[x, y]:
        return ""
    tile_name = ""
    for key,value in tile_colors.items():
        if (game_map.tiles[x][y][3][2] == value).all():
            print(str(game_map.tiles[x][y][3][2]))
            print(value)
            tile_name = key

    return tile_name.capitalize()
...
def render_tile_at_mouse_location(console: Console, x: int, y: int, engine: Engine) -> None:
    mouse_x, mouse_y = engine.mouse_location

    names_at_mouse_location = get_tile_names_at_location(x=mouse_x, y=mouse_y, game_map=engine.game_map)
